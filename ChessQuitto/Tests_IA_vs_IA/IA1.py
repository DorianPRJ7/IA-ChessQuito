...
def valMaxPlacement1(jeu, mode_jeu, couleur_ia, couleur_humain, pieces_ia, pieces_humain, alpha, beta, profondeur):
    """
    ...
    if (profondeur==0) or (len(pieces_ia) == 0) or (len(lesCoups) == 0):
        ...
        return evaluer_placement1(jeu, mode_jeu, couleur_ia), '-f', '-f'
    """
        Algorithme :: PVH
        ...
        for piece in pieces_ia:
            if (mode_jeu!=3) or (len(pieces_ia)<4) or (piece[1:]=="RR"): # Si on est pas en mode 3 OU qu'on a moins de 4 pieces a poser OU qu'on veut poser le Roi, alors on teste
                ...
                if (mode_jeu!=3) or (not roi_en_echec(nouveauJeu, couleur_humain) and not roi_en_echec(nouveauJeu, couleur_ia)): # Si on est pas en mode 3 OU qu'on ne met pas le roi adverse en echec ET que le mien non plus, alors
                    score, _, _ = valMinPlacement1(nouveauJeu, mode_jeu, couleur_humain, couleur_ia, pieces_humain, nouvellesPieces, alpha, beta, profondeur-1)
...
def valMinPlacement1(jeu, mode_jeu, couleur_humain, couleur_ia, pieces_humain, pieces_ia, alpha, beta, profondeur):
    """
    ...
    if (profondeur == 0) or (len(pieces_humain) == 0) or (len(lesCoups) == 0):
        ...
        return evaluer_placement1(jeu, mode_jeu, couleur_humain), '-f', '-f'
        ...
        for piece in pieces_humain:
            if (mode_jeu != 3) or (len(pieces_humain) < 4) or (piece[1:] == "RR"):  # Si on est pas en mode 3 OU qu'on a moins de 4 pieces a poser OU qu'on veut poser le Roi, alors on teste
                ...
                if (mode_jeu != 3) or (not roi_en_echec(nouveauJeu, couleur_ia) and not roi_en_echec(nouveauJeu, couleur_humain)):  # Si on est pas en mode 3 OU qu'on ne met pas le roi adverse en echec ET que le mien non plus, alors
                    score, _, _ = valMaxPlacement1(nouveauJeu, mode_jeu,couleur_ia, couleur_humain, pieces_ia, nouvellesPieces, alpha, beta, profondeur-1)
...
def lancer_tour_placement_ia1(jeu, mode_jeu, couleur_ia, couleur_humain, pieces_ia, pieces_humain):
    best_score, best_pos, best_piece = valMaxPlacement1(jeu, mode_jeu,couleur_ia,couleur_humain, pieces_ia, pieces_humain, -math.inf, +math.inf, profondeur=3)
...
############## PHASE DE JEU ##############
def evaluer_jeu1(jeu, mode_jeu, couleur_joueur):
    if mode_jeu == 1:
        pond_score = 2.0
        pond_malus = 1.2
        pond_position = 1.0
...
def valMaxJeu1(jeu, mode_jeu, sans_prise, couleur_ia, couleur_humain, alpha, beta, profondeur):
    """
        Fonction recursive appelée par Machine
    """
    if (profondeur==0) or (verifierFinPartie(mode_jeu,jeu,sans_prise)):
        # print("pieces_ia=", pieces_ia) # DEBUG
        return evaluer_jeu1(jeu, mode_jeu, couleur_ia), '-f', '-f'
    """
        Algorithme :: PVH
        Hypothèse : score en deçà du minimum
        Vérification : à chaque coup, màj de scoreMax et coupMax si besoin
    ...
    for pos in allies:
        ...
        for coup in lesCoups:
            ...
            if (mode_jeu!=3) or (not roi_en_echec(nouveauJeu, couleur_ia)): # Si on est pas en mode 3 OU qu'on n'a pas mis notre roi en echec, alors on continue
                ...
                score, _, _ = valMinJeu1(nouveauJeu, mode_jeu, sans_prise, couleur_ia, couleur_humain, alpha, beta, profondeur-1)
...
def valMinJeu1(jeu, mode_jeu, sans_prise, couleur_ia, couleur_humain, alpha, beta, profondeur):
    """
        Fonction recursive appelée par Machine
    """
    if (profondeur==0) or (verifierFinPartie(mode_jeu,jeu,sans_prise)):
        # print("pieces_ia=", pieces_ia) # DEBUG
        return evaluer_jeu1(jeu, mode_jeu, couleur_humain), '-f', '-f'
    """
        Algorithme :: PVH
        Hypothèse : score en deçà du minimum
        Vérification : à chaque coup, màj de scoreMax et coupMax si besoin
    ...
    for pos in allies:
        ...
        for coup in lesCoups:
            ...
            if (mode_jeu!=3) or (not roi_en_echec(nouveauJeu, couleur_humain)): # Si on est pas en mode 3 OU qu'on n'a pas mis notre roi en echec, alors on continue
                ...
                score, _, _ = valMaxJeu1(nouveauJeu, mode_jeu, sans_prise, couleur_ia, couleur_humain, alpha, beta, profondeur-1)
...
def lancer_tour_jeu_ia1(jeu, mode_jeu, sans_prise, couleur_ia, couleur_adv):
    best_score, best_pos, best_piece = valMaxJeu1(jeu, mode_jeu, sans_prise, couleur_ia, couleur_adv, -math.inf, +math.inf, profondeur=3)
    if best_pos == '-f' or best_piece == '-f':
        return
