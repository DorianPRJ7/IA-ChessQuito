...
def evaluer_placement1(jeu, mode_jeu, couleur_joueur):
    ...
    pond_score = 1.0
    pond_malus = 1.0  # ↑ prendre les menaces plus au sérieux
    pond_position = 0.7  # ↑ un peu plus de stratégie spatiale
    pond_attaque = 0.8  # ↓ moins d'agressivité aveugle
    pond_soutien = 1.0  # ↑ énorme focus sur soutien mutuel
    pond_mobilite = 0.5  # ↑ pour inciter aux placements dynamiques
    pond_diversite = 0.4  # léger up
    pond_penalite_proximite = 0.5  # ↑ éviter les pièces trop collées
    pond_proximite_roi = 1.4  # ↑ roi protégé = priorité en mode 3
...
def valMaxPlacement1(jeu, mode_jeu, couleur_ia1, couleur_ia2, pieces_ia1, pieces_ia2, alpha, beta, profondeur):
    """
    ...
    if (profondeur==0) or (len(pieces_ia1) == 0) or (len(lesCoups) == 0):
        ...
        return evaluer_placement1(jeu, mode_jeu, couleur_ia1), '-f', '-f'
    """
        Algorithme :: PVH
        ...
        for piece in pieces_ia1:
            if (mode_jeu!=3) or (len(pieces_ia1)<4) or (piece[1:]=="RR"): # Si on est pas en mode 3 OU qu'on a moins de 4 pieces a poser OU qu'on veut poser le Roi, alors on teste
                ...
                if (mode_jeu!=3) or (not roi_adverse_en_echec(nouveauJeu, couleur_ia2,pieces_ia2) and not roi_adverse_en_echec(nouveauJeu, couleur_ia1, nouvellesPieces)): # Si on est pas en mode 3 OU qu'on ne met pas le roi adverse en echec ET que le mien non plus, alors
                    score, _, _ = valMinPlacement1(nouveauJeu, mode_jeu, couleur_ia2, couleur_ia1, pieces_ia2, nouvellesPieces, alpha, beta, profondeur-1)
                    # DEBUG
...
def valMinPlacement1(jeu, mode_jeu, couleur_ia2, couleur_ia1, pieces_ia2, pieces_ia1, alpha, beta, profondeur):
    """
    ...
    if (profondeur == 0) or (len(pieces_ia2) == 0) or (len(lesCoups) == 0):
        ...
        return evaluer_placement1(jeu, mode_jeu, couleur_ia2), '-f', '-f'
        ...
        for piece in pieces_ia2:
            if (mode_jeu != 3) or (len(pieces_ia2) < 4) or (piece[1:] == "RR"):  # Si on est pas en mode 3 OU qu'on a moins de 4 pieces a poser OU qu'on veut poser le Roi, alors on teste
                ...
                if (mode_jeu != 3) or (not roi_adverse_en_echec(nouveauJeu, couleur_ia1,pieces_ia1)  and not roi_adverse_en_echec(nouveauJeu, couleur_ia2, nouvellesPieces)):  # Si on est pas en mode 3 OU qu'on ne met pas le roi adverse en echec ET que le mien non plus, alors
                    score, _, _ = valMaxPlacement1(nouveauJeu, mode_jeu,couleur_ia1, couleur_ia2, pieces_ia1, nouvellesPieces, alpha, beta, profondeur-1)
...
def lancer_tour_placement_ia1(jeu, mode_jeu, couleur_ia1, couleur_ia2, pieces_ia1, pieces_ia2):
    best_score, best_pos, best_piece = valMaxPlacement1(jeu, mode_jeu,couleur_ia1,couleur_ia2, pieces_ia1, pieces_ia2, -math.inf, +math.inf, profondeur=3)
