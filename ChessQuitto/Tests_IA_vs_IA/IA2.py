...
def evaluer_placement2(jeu, mode_jeu, couleur_joueur):
    ...
    pond_score=1.0
    pond_malus=1.1 # 0.7
    pond_position=0.8 # 0.6
    pond_attaque=0.7 # 1.2
    pond_soutien=1.2 # 0.5
    pond_mobilite=0.6 # 0.3
    pond_diversite=0.5 # 0.3
    pond_penalite_proximite=0.6 # 0.2
    pond_proximite_roi=1.6 # 1.2
...
def valMaxPlacement2(jeu, mode_jeu, couleur_ia1, couleur_ia2, pieces_ia1, pieces_ia2, alpha, beta, profondeur):
    """
    ...
    if (profondeur==0) or (len(pieces_ia1) == 0) or (len(lesCoups) == 0):
        ...
        return evaluer_placement2(jeu, mode_jeu, couleur_ia1), '-f', '-f'
    """
        Algorithme :: PVH
        ...
        for piece in pieces_ia1:
            if (mode_jeu!=3) or (len(pieces_ia1)<4) or (piece[1:]=="RR"): # Si on est pas en mode 3 OU qu'on a moins de 4 pieces a poser OU qu'on veut poser le Roi, alors on teste
                ...
                if (mode_jeu!=3) or (not roi_adverse_en_echec(nouveauJeu, couleur_ia2,pieces_ia2) and not roi_adverse_en_echec(nouveauJeu, couleur_ia1, nouvellesPieces)): # Si on est pas en mode 3 OU qu'on ne met pas le roi adverse en echec ET que le mien non plus, alors
                    score, _, _ = valMinPlacement2(nouveauJeu, mode_jeu, couleur_ia2, couleur_ia1, pieces_ia2, nouvellesPieces, alpha, beta, profondeur-1)
                    # DEBUG
...
def valMinPlacement2(jeu, mode_jeu, couleur_ia2, couleur_ia1, pieces_ia2, pieces_ia1, alpha, beta, profondeur):
    """
    ...
    if (profondeur == 0) or (len(pieces_ia2) == 0) or (len(lesCoups) == 0):
        ...
        return evaluer_placement2(jeu, mode_jeu, couleur_ia2), '-f', '-f'
        ...
        for piece in pieces_ia2:
            if (mode_jeu != 3) or (len(pieces_ia2) < 4) or (piece[1:] == "RR"):  # Si on est pas en mode 3 OU qu'on a moins de 4 pieces a poser OU qu'on veut poser le Roi, alors on teste
                ...
                if (mode_jeu != 3) or (not roi_adverse_en_echec(nouveauJeu, couleur_ia1,pieces_ia1)  and not roi_adverse_en_echec(nouveauJeu, couleur_ia2, nouvellesPieces)):  # Si on est pas en mode 3 OU qu'on ne met pas le roi adverse en echec ET que le mien non plus, alors
                    score, _, _ = valMaxPlacement2(nouveauJeu, mode_jeu,couleur_ia1, couleur_ia2, pieces_ia1, nouvellesPieces, alpha, beta, profondeur-1)
...
def lancer_tour_placement_ia2(jeu, mode_jeu, couleur_ia1, couleur_ia2, pieces_ia1, pieces_ia2):
    best_score, best_pos, best_piece = valMaxPlacement2(jeu, mode_jeu,couleur_ia1,couleur_ia2, pieces_ia1, pieces_ia2, -math.inf, +math.inf, profondeur=3)
